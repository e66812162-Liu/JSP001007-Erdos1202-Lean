#!/usr/bin/env bash
set -uo pipefail

mkdir -p jsp_logs

files=(
  JSP001007_WP4N_DyadicPNTLowerBound_V5.lean
  JSP001007_ClusterGapEngine1202_Candidate.lean
  JSP001007_WP4C_PrimeBoxPigeonhole_StaticAuditV4.lean
  JSP001007_WP4C_Arch51_PrimeBox_StaticAuditV4.lean
  JSP001007_WP4P_ClusterToCounterexample_Kernel_V6.lean
  JSP001007_WP4P_PNT_to_ClusterData_Adapter_V4.lean
  JSP001007_WP4Q_SourceFaithful_FinalWrapper_V13.lean
  JSP001007_Master_SourceFaithful_V13.lean
)

echo "=== Environment ===" | tee jsp_logs/00_environment.log
echo "PrimeGapsLib HEAD: $(git rev-parse HEAD)" | tee -a jsp_logs/00_environment.log
echo "Lean toolchain: $(cat lean-toolchain)" | tee -a jsp_logs/00_environment.log
lake env lean --version 2>&1 | tee -a jsp_logs/00_environment.log || true

mkdir -p .lake/build/lib/lean

echo "=== Building required PrimeGapsTheory modules ===" | tee jsp_logs/01_primegaps_build.log
set +e
lake build PrimeGapsTheory.NumberTheory.DyadicPNT \
  PrimeGapsTheory.NumberTheory.PrimeCountingInterval \
  2>&1 | tee -a jsp_logs/01_primegaps_build.log
dep_rc=${PIPESTATUS[0]}
set -e
if [ "$dep_rc" -ne 0 ]; then
  echo "DEPENDENCY BUILD FAILED" | tee jsp_logs/FIRST_FAILURE.txt
  exit "$dep_rc"
fi

status=0
for f in "${files[@]}"; do
  log="jsp_logs/${f%.lean}.log"
  echo "COMPILING $f" | tee "$log"
  set +e
  out=".lake/build/lib/lean/${f%.lean}.olean"
  lake env lean -o "$out" "$f" 2>&1 | tee -a "$log"
  rc=${PIPESTATUS[0]}
  set -e
  if [ "$rc" -ne 0 ]; then
    echo "FIRST FAILING MODULE: $f" | tee jsp_logs/FIRST_FAILURE.txt
    status=$rc
    break
  fi
done

if [ "$status" -eq 0 ]; then
  echo "ALL V13 CANDIDATE MODULES COMPILED" | tee jsp_logs/GREEN.txt
fi

exit "$status"
